...
2. Установка **ffmpeg** and **ffprobe**
    ...
    * [windows](https://ffmpeg.zeranoe.com/builds/) скачать и переместить `ffmpeg.exe`, `ffprobe.exe` в **папку со скриптом**
    * linux `apt-get install ffmpeg`

3. Переменные
   * `TELETHON_CITY` (название города)

   * `TELETHON_API_HASH` (получить [api_hash](https://my.telegram.org/auth?to=apps))

   * `TELETHON_API_ID` (получить можно так же как и api_hash)
